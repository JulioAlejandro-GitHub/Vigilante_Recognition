...
class Settings(BaseSettings):
    ...
    # Reconocimiento
    enable_recognition_camera_suppression: bool = Field(default=True, description="Evita reprocesamiento en BD de la misma persona en la misma cámara en X segundos")
    recognition_camera_suppression_seconds: int = Field(default=30, description="Segundos de ventana de supresión por cámara")
    ...
    # Identidades Conocidas (Enrolados)
    ...
    known_identity_max_embeddings: int = Field(default=5, description="Número máximo de embeddings a guardar por persona conocida")
    known_identity_use_centroid: bool = Field(default=True, description="Habilita el uso de centroides para personas conocidas")
    known_identity_representative_policy: str = Field(default="best_quality", description="Política para seleccionar embeddings representativos")
    known_identity_match_mode: str = Field(default="centroid_plus_topk", description="Modo de matching contra la galería de conocidos (centroid, topk, centroid_plus_topk)")
    ...
    observed_identity_use_centroid: bool = Field(default=True, description="Habilita el uso de centroides para identidades observadas")
    observed_identity_duplicate_similarity_threshold: float = Field(default=0.95, description="Umbral de similitud para considerar un embedding como duplicado")
    observed_identity_representative_policy: str = Field(default="best_quality", description="Política para seleccionar embeddings representativos")
    observed_identity_match_mode: str = Field(default="centroid_plus_topk", description="Modo de matching contra la galería observada (centroid, topk, centroid_plus_topk)")
